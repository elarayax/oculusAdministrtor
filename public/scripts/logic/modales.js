...
function printModal(impresoras, contenido, metodoImpresion) {
    ...
    let impresoraSeleccionada = null; // Variable local para la impresora seleccionada
    ...
    botonImprimir.addEventListener('click', () => {
        if (impresoraSeleccionada) {
            metodoImpresion(impresoraSeleccionada, contenido); // Llamar al método dinámico
        } else {
...
// Ejemplo de función de impresión personalizada
async function imprimirAhora(impresora, contenido) {
    console.log("Imprimiendo en la impresora", impresora.nombre);
...
async function imprimirInformes(impresora, contenido){
    console.log("Imprimiendo en la impresora", impresora.nombre);
    ...
    try {
        const result = await imprimirInforme(ticketData);

        if (result) {
            generarMensaje("green", "Informe impreso correctamente:", result.message);
            console.log(result.data);
        } else {
            generarMensaje("red", "No se pudo imprimir el informe.");
        }
    } catch (error) {
        generarMensaje("red", `Error al imprimir: ${error.message}`);
        console.log(error);
...
async function imprimirInformesSemanales(impresora, contenido){
    console.log("Imprimiendo en la impresora", impresora.nombre);
    ...
    try {
        const result = await imprimirInformeSemanal(ticketData);
