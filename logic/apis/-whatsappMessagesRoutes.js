...
module.exports = (server) => {
    server.get('/get-whatsapp-qr', async (req, res) => {
        try {
            if (qrCodeData) {
                res.json({ qr: `data:image/png;base64,${qrCodeData}` }); // Envía el QR como base64
            } else {
    ...
    server.post('/clear-local-auth', (req, res) => {
        clearLocalAuth();
        res.status(200).json({ message: 'Sesión de WhatsApp eliminada. Se requiere nuevo QR.' });
    });

    server.post('/send-whatsapp-message', async (req, res) => {
    ...
    server.get('/whatsapp-status', (req, res) => {
        if (client.info) {
    ...
    server.post('/send-whatsapp-image', async (req, res) => {
